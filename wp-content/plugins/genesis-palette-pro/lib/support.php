...
class GP_Pro_Support
{
	...
	public static function support_missing() {
		...
		// Create my Genesis settings license link.
		$link   = GP_Pro_Licensing::get_license_page_link();

		// Return the markup directing the user to the license field.
		return '<div class="gppro-input gppro-support-input"><p>' . sprintf( __( 'You must have a valid license key to recieve support. <a href="%s">Click here to enter your license key.</a>', 'gppro' ), esc_url( $link ) ) . '</p></div>';
	}
	...
	public static function support_widget() {
		...
		// Create my Genesis settings license link.
		$enter  = GP_Pro_Licensing::get_license_page_link();
		...
		// Add the link for the license field.
		$input .= '<div class="gppro-input gppro-support-input"><p><strong>' . sprintf( __( 'Looking for your license key information? <a href="%s">Click here.</a>', 'gppro' ), esc_url( $enter ) ) . '</strong></p></div>';
	...
	public function manual_support_request() {
		...
		if ( empty( $_POST['gppro_support_nonce'] ) || ! wp_verify_nonce( $_POST['gppro_support_nonce'], 'gppro_support_nonce' ) ) {
			...
			$link   = add_query_arg( array( 'action' => 'support', 'processed' => 'failure', 'errcode' => 'MISSING_NONCE' ), $base );
		...
		if ( empty( $_POST['gppro-support-name'] ) ) {
			...
			// Set my redirect link with the error code.
			$link   = add_query_arg( array( 'action' => 'support', 'processed' => 'failure', 'errcode' => 'MISSING_NAME' ), $base );
		...
		if ( empty( $_POST['gppro-support-email'] ) ) {
			...
			// Set my redirect link with the error code.
			$link   = add_query_arg( array( 'action' => 'support', 'processed' => 'failure', 'errcode' => 'MISSING_EMAIL' ), $base );
		...
		if ( ! is_email( $_POST['gppro-support-email'] ) ) {
			...
			// Set my redirect link with the error code.
			$link   = add_query_arg( array( 'action' => 'support', 'processed' => 'failure', 'errcode' => 'INVALID_EMAIL' ), $base );
		...
		if ( empty( $_POST['gppro-support-text'] ) ) {
			...
			// Set my redirect link with the error code.
			$link   = add_query_arg( array( 'action' => 'support', 'processed' => 'failure', 'errcode' => 'MISSING_TEXT' ), $base );
		...
		if ( false === $details = self::get_help_details( $data ) ) {
			...
			// Set my redirect link with the error code.
			$link   = add_query_arg( array( 'action' => 'support', 'processed' => 'failure', 'errcode' => 'NO_DETAILS' ), $base );
		...
		if ( false !== $process = self::process_help_api( $details ) ) {
			...
			$link   = add_query_arg( array( 'action' => 'support', 'processed' => 'success', 'method' => 'api' ), $base );
		...
		if ( false !== $fallback = self::process_help_email( $details ) ) {
			...
			// Set my redirect link with the success.
			$link   = add_query_arg( array( 'action' => 'support', 'processed' => 'success', 'method' => 'email' ), $base );
		...
		if ( false === $process && false === $fallback ) {
			...
			// Set my redirect link with the error code.
			$link   = add_query_arg( array( 'action' => 'support', 'processed' => 'failure', 'errcode' => 'SEND_FAILED' ), $base );
